...
def draw_polygons( polygons, screen, zbuffer, view, ambient, light, areflect, dreflect, sreflect):
    ...
    while point < len(polygons) - 2:
        ...
        if normal[2] > 0:
            normalize(normal)
            # print(polygons[point])
            direction = [polygons[point][0]-light[LOCATION][0],polygons[point][1]-light[LOCATION][1],polygons[point][2]-light[LOCATION][2]]
            # direction = [int(x*(-1)) for x in direction]
            light_copy = [[direction[0],direction[1],direction[2]],[light[COLOR][0],light[COLOR][1],light[COLOR][2]]]
            
            normalize(light_copy[LOCATION])
            color = get_lighting(normal, view, ambient, light_copy, areflect, dreflect, sreflect )
